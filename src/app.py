...
Includes five pages: analytics dashboard, cohort and funnel retention, data submission,
...
page = st.sidebar.radio(
    "Navigate",
    ["📊 Dashboard", "🔄 Cohorts & Funnels", "📥 Submit Data", "🔍 Data Quality", "🤖 Ask AI"],
...
# =====================================================================
# Page 2: Cohorts & Funnels
# =====================================================================
elif page == "🔄 Cohorts & Funnels":

    st.markdown("""<div class="top-banner">
        <h1>Retention Cohorts & Sales Funnels</h1>
        <p>Analyze customer repeat purchase quarterly cohorts and storefront traffic conversion pipelines</p>
    </div>""", unsafe_allow_html=True)

    tab_retention, tab_funnel = st.tabs(["🔄 Customer Purchase Retention", "🛒 E-Commerce Clickstream Funnel"])

    # Filters for Cohorts & Funnels
    yrs, mkts, segs = get_filters()
    st.sidebar.markdown("### Filters")
    yr = st.sidebar.selectbox("Year", yrs)
    mkt = st.sidebar.selectbox("Market", mkts)
    ...
    with tab_retention:
        st.write("#### Customer Purchase Retention (Cohort Analysis)")
        st.caption("Tracks repeat order behavior across quarterly acquisition cohorts. Standard retail metrics use cohorts based on customer signup (first order date) quarters to measure customer lifetime value (LTV).")
        ...
        ret_df = query_db(retention_query, params * 2)
        ...
        if not ret_df.empty:
            ...
            if pd.isna(avg_q1_ret): avg_q1_ret = 0.0
            if pd.isna(avg_q4_ret): avg_q4_ret = 0.0

            st.markdown(f"""<div class="stats-row">
                <div class="stat-box teal">
                    <div class="stat-label">Avg Quarter 1 Retention</div>
                    <div class="stat-num">{avg_q1_ret:.1f}%</div>
                    <div class="stat-hint">returning buyers after Q0</div>
                </div>
                <div class="stat-box coral">
                    <div class="stat-label">Avg Quarter 4 Retention</div>
                    <div class="stat-num">{avg_q4_ret:.1f}%</div>
                    <div class="stat-hint">retention at Year 1</div>
                </div>
                <div class="stat-box purple">
                    <div class="stat-label">Total Cohorts Tracked</div>
                    <div class="stat-num">{total_cohorts}</div>
                    <div class="stat-hint">quarterly signup cohorts</div>
                </div>
            </div>""", unsafe_allow_html=True)
            ...
            st.markdown("""<div class="chart-wrap">
                <h3>Customer Logo Retention Heatmap (%)</h3>
                <div class="subtitle">Cohort purchase rates by quarter elapsed</div>
            """, unsafe_allow_html=True)
            ...
            fig_ret.update_layout(**chart_style(
                height=400,
            ...
            st.markdown("""<div class="stats-row">
                <div class="stat-box teal" style="background:#f8fafc; border-left-color: #6366f1;">
                    <div class="stat-label" style="color:#6366f1; font-weight:700;">Loyalty Base Durability</div>
                ...
                <div class="stat-box purple" style="background:#f8fafc; border-left-color: #8b5cf6;">
                    <div class="stat-label" style="color:#8b5cf6; font-weight:700;">Long-Tail Retention Stabilisation</div>
                    <div style="font-size:0.92rem; color:#475569; margin-top:6px; line-height:1.5;">
    ...
    with tab_funnel:
        st.write("#### E-Commerce Clickstream & Conversion Funnel")
        st.caption("Maps visitor clickstream actions on the digital storefront against actual database order purchases, illustrating conversion drops and UX opportunities.")
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        FROM BaseStats;
        """
        fun_df = query_db(funnel_query, params)
        ...
        if not fun_df.empty and fun_df['visitor_count'].iloc[0] > 0:
            ...
            st.markdown(f"""<div class="stats-row">
                <div class="stat-box teal">
                    <div class="stat-label">Session-to-Order Conversion</div>
                    <div class="stat-num">{conversion_rate_fun:.2f}%</div>
                    <div class="stat-hint">overall conversion rate</div>
                </div>
                <div class="stat-box green">
                    <div class="stat-label">Total Sales Generated</div>
                    <div class="stat-num">${total_sales_fun:,.0f}</div>
                    <div class="stat-hint">implied conversion value</div>
                </div>
                <div class="stat-box purple">
                    <div class="stat-label">Total Store Sessions</div>
                    <div class="stat-num">{total_sessions:,}</div>
                    <div class="stat-hint">captured traffic volume</div>
                </div>
            </div>""", unsafe_allow_html=True)
            ...
            with col_graph:
                st.markdown("""<div class="chart-wrap">
                    <h3>E-Commerce Storefront Funnel Chart</h3>
                    <div class="subtitle">Conversion volumes through shopping stages</div>
                """, unsafe_allow_html=True)
                fig_fun = go.Figure(go.Funnel(
                    ...
                    opacity=0.85,
                    marker={"color": ["#6366f1", "#4f46e5", "#3b82f6", "#14b8a6", "#22c55e"]},
                    connector={"line": {"color": "rgba(0,0,0,0.1)", "width": 1}}
                ))
                fig_fun.update_layout(**chart_style(
                    height=360,
            ...
            with col_table:
                st.markdown("""<div class="chart-wrap">
                    <h3>Funnel Phase Details</h3>
                    <div class="subtitle">Step-by-step conversion and dropout details</div>
                """, unsafe_allow_html=True)
                st.dataframe(
            ...
            st.markdown("""<div class="stats-row">
                <div class="stat-box teal" style="background:#f8fafc; border-left-color: #e11d48;">
                    <div class="stat-label" style="color:#e11d48; font-weight:700;">Product Interest to Cart Friction</div>
                    <div style="font-size:0.92rem; color:#475569; margin-top:6px; line-height:1.5;">
                        The session-to-purchase conversion stands at 5.4%, aligning with global e-commerce averages.
                ...
                <div class="stat-box green" style="background:#f8fafc; border-left-color: #16a34a;">
                    <div class="stat-label" style="color:#16a34a; font-weight:700;">High Checkout-to-Purchase Intent</div>
                    <div style="font-size:0.92rem; color:#475569; margin-top:6px; line-height:1.5;">
                        Once users initiate the checkout stage, conversion rate rises sharply: 58.8% of users who initiate checkout complete their purchase. 
